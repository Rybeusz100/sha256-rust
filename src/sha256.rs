...
pub fn hash_file(path: &Path, buf_size_kb: usize) -> Result<String, std::io::Error> {
    ...
    let last_chunk_start = file_size_bytes - ((buf_reads - 1) * buf_size_bytes as u64) - (file_size_bytes % 64);
    ...
    let chunks_in_buf;
    if data_usize + 1 + bytes_to_append + 8 > 64 {
        chunks_in_buf = 2;
    }
    else {
        chunks_in_buf = 1;
    ...
    // return hash
    let hash_string = format!("{:08x}{:08x}{:08x}{:08x}{:08x}{:08x}{:08x}{:08x}", hash[0], hash[1], hash[2], hash[3], hash[4], hash[5], hash[6], hash[7]);
    Ok(hash_string)
}